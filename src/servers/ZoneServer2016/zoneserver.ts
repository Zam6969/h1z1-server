...
export class ZoneServer2016 extends EventEmitter {
  ...
  async unbanClient(
    ...
    if(!unBannedClient.loginSessionId) return;
  ...
  useAirdrop(client: Client, item: BaseItem): void {
    // Bypass restrictions if client.isDebugMode is active
    if (client.isDebugMode) {
      this.sendAlert(client, "Debug mode is active. Bypassing restrictions.");
    } else {
      // Rest of the code with existing restrictions...
      if (this._airdrop) {
      ...
      if (
        ...
        this.sendAlert(client, "Invalid airdrop item.");
        return;
      }
    }
  
    // Code to use the airdrop
    ...
    // Rest of the code to set up the airdrop...
